...
- **Selection rules (mechanical, to prevent drift into stock-tipping):** S&P 500 constituents with |1D| ≥ 4%; top-50 mega-caps with |1D| ≥ 2.5%; any GICS sector ETF diverging ≥ 1.5pp from SPX on the day. Earnings/guidance reactions prioritized over drift. **Cap: 3 names + 1 sector note.** Source: FMP (gainers/losers, sector performance); web search with named-source attribution for the reported reason *(amended 2026-08-21 by Jacob: Bigdata.com removed — no paid services; the system runs on daily closes, and attributed web sourcing proved sufficient across the first three editions)*.
...
- Daily: SMH relative strength vs. SPY; any credit-market news on hyperscaler/neocloud issuance, vendor financing structures, datacenter debt (web search with named-source attribution — *amended 2026-08-21: Bigdata.com removed per the no-paid-services decision; if issuance coverage proves too thin at web-search depth, raise it at monthly review rather than restoring a paid source silently*).
